...
When asked to *propose* refactorings rather than apply them, change nothing and return:

1. A concise summary of the main complexity / token-cost issues.
2. A prioritized list of refactorings. For each: why it helps, which files and functions are affected, whether it is low / medium / high risk, and whether it can be done independently of the others.
3. Files that should be split, merged, renamed, or documented.
4. A suggested implementation order suited to future agent sessions, sequenced so each step is independently reviewable.

Alongside structural issues, call out: functions that are too large or do too much, weak naming, poor separation of concerns, missing comments around business rules / assumptions / edge cases / non-obvious logic, and code that creates excessive cross-file dependency chains.

## Redundant-code sweep

When scanning for redundant, unused, or duplicated code (functions, classes, files, imports, commented-out blocks):

- For each candidate, explain why it appears redundant and cite the exact location and snippet.
- Remove it locally, then run the verification commands below. If anything fails, **revert that change** and report the failure details and stack traces — never weaken a test or leave the tree broken to make a removal stick.
- If behaviour or intent is unclear, do not modify. Present the candidate, the risk of removal, and a recommended action (safe removal, refactor, or ask the developer).
- Give a one-line summary and the exact diff for each proposed change.
- Prefer the conservative option when several interpretations exist, and ask before any non-trivial behavioural change.

## Verification commands (this repo)

This is a TypeScript / Vitest / npm repository. Use its own commands — there is no Python, Maven, or Gradle tooling here:

- `npm test` — fast smoke suite
- `npm run test:all` — both unit-test groups
- `npm run typecheck` — core, client, and server type-checks
- `npm run lint` — ESLint over `src`, `shared`, `server`, `client`
- `npm run deps:check` — import-boundary / dependency-graph check after moving files

Playwright (`npm run e2e`) is an approval-only escalation; do not run it as a default check.
