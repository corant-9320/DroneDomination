...
- **Refineries** — cover a whole hex; consume raw Oil and produce Refined_Product; grow in productivity as the player adds more refinery segments in the hex.
...
- **Refinery**: A structure that covers one entire HexTile and processes raw Oil into Refined_Product. Composed of one or more Refinery_Segments.
- **Refinery_Segment**: A single building placed on one HexSegment of a Refinery HexTile. The number of Refinery_Segments determines the Refinery's processing throughput.
...
**User Story:** As a player, I want refineries that turn raw oil into a refined product and that I can expand by building more segments, so that I can invest to increase the output that fuels my construction.

#### Acceptance Criteria

1. WHERE a player selects an eligible HexTile that does not already contain a Refinery, THE Logistics_System SHALL allow the player to build a Refinery that covers all HexSegments of that HexTile.
2. WHEN a Refinery is first built, THE Logistics_System SHALL create exactly one Refinery_Segment on exactly one HexSegment of the Refinery HexTile.
3. WHERE a Refinery HexTile contains at least one HexSegment that is not occupied by a Refinery_Segment, THE Logistics_System SHALL allow the player to build one additional Refinery_Segment on an unoccupied HexSegment, up to a maximum of one Refinery_Segment per HexSegment in the HexTile (5 or 6 segments).
4. THE Logistics_System SHALL set each Refinery's processing throughput equal to the number of Refinery_Segments it contains multiplied by the Refinery_Throughput_Rate of 20 raw Oil units per Refinery_Segment per turn, such that a Refinery containing N Refinery_Segments processes exactly N times 20 raw Oil per turn.
5. WHILE a Refinery holds raw Oil and has available throughput in a turn, THE Logistics_System SHALL, once per turn, consume a quantity of raw Oil equal to the lesser of the Refinery's remaining throughput for that turn and the raw Oil available to that Refinery, and produce Refined_Product equal to the consumed raw Oil multiplied by the Conversion_Ratio of 0.5 (2 raw Oil units yield 1 Refined_Product unit), rounding any fractional Refined_Product down to the nearest whole unit.
6. WHEN a Refinery produces Refined_Product, THE Logistics_System SHALL decrease that Refinery's held raw Oil by the quantity consumed and SHALL make the produced Refined_Product available for transport from that Refinery.
7. IF a Refinery has no raw Oil available at the start of a turn, THEN THE Logistics_System SHALL produce zero Refined_Product at that Refinery for that turn and SHALL leave the Refinery's held raw Oil unchanged.
8. IF a player attempts to build a Refinery_Segment on a HexSegment outside the Refinery's HexTile, THEN THE Logistics_System SHALL reject the construction, leave the target HexSegment unchanged, and return an indication that the segment is outside the Refinery HexTile.
9. IF a player attempts to build an additional Refinery_Segment on a Refinery HexTile in which every HexSegment is already occupied by a Refinery_Segment, THEN THE Logistics_System SHALL reject the construction, leave all HexSegments unchanged, and return an indication that the Refinery is at maximum segment capacity.
10. IF a player attempts to build a Refinery on a HexTile that is not eligible or that already contains a Refinery, THEN THE Logistics_System SHALL reject the construction, leave the HexTile unchanged, and return an indication that the HexTile cannot host a new Refinery.
11. THE Logistics_System SHALL treat a HexTile as eligible to host a new Refinery only WHEN that HexTile is land owned by the requesting player, every HexSegment of that HexTile has a Segment_Steepness at or below the Tank_Steepness_Threshold (MAX_STEEP_WHEELED = 0.44 radians), and no HexSegment of that HexTile is occupied by an Oil_Well, a Refinery_Segment, or another building.
12. IF a player attempts to build a Refinery on a HexTile classified as water or as a Forest_Tile whose trees have not been cleared, THEN THE Logistics_System SHALL reject the construction, leave the HexTile unchanged, and return an indication that the terrain cannot host a Refinery.
...
No open questions remain. All previously open items have been resolved with concrete decisions and encoded as acceptance criteria and Glossary values (see Resolved Questions below).
