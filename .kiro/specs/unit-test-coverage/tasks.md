...
- [ ] 1. Test tooling setup
  - [ ] 1.1 Add and pin `fast-check` as a dev dependency
...
- [ ] 2. P1 — Combat-correctness coverage (new)
  - [ ] 2.1 Add behavioural coverage for `src/world/combatFacing.ts`
  ...
  - [ ] 2.2 Add behavioural coverage for `src/world/combatFormula.ts`
  ...
  - [ ] 2.3 Add behavioural coverage for `shared/rangeCheck.ts`
...
- [ ] 3. Convert repair formula tests to property assertions
  - [ ] 3.1 Rewrite pinned values in `src/world/__tests__/repair.test.ts`
...
- [ ] 4. Review-and-repair pass over existing in-scope tests
  - [ ] 4.1 Review and repair existing `src/world/__tests__` tests
  ...
  - [ ] 4.2 Review and repair existing `server/__tests__` tests
...
- [ ] 6. P2 — Server combat coverage
  - [ ] 6.1 Extend coverage for `server/combatApi.ts`
  ...
  - [ ] 6.2 Add behavioural coverage for `server/combatExplainer.ts`
...
- [ ] 7. P3 — Remaining-gap coverage
  - [ ] 7.1 Add round-trip coverage for `src/world/compact.ts`
  ...
  - [ ] 7.2 Add coverage for `src/world/geodesic.ts` and `src/world/segmentGeometry.ts`
  ...
  - [ ] 7.3 Extend/add coverage for `src/world/pathfinding.ts` and `shared/pathfinding.ts`
  ...
  - [ ] 7.4 Add coverage for `src/world/validate.ts`, `shared/unitNaming.ts`, and `src/world/buildings.ts`
  ...
  - [ ] 7.5 Add integration/smoke coverage for generation and server wiring
...
```json
{
  "waves": [
    ...
    { "id": 2, "tasks": ["6.1", "6.2", "7.1", "7.2", "7.3", "7.4", "7.5"] },
    { "id": 3, "tasks": ["8.1"] },
    { "id": 4, "tasks": ["8.2"] }
  ]
}
