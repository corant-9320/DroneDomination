...
of the sparse `battle-20v20.json`. The new scenario is generated by
`scripts/generate-default-scenario.js` and is built for testing every system at
once: two big cities (player capital `city_0` @2123 and enemy capital `city_6`
@32003) each owning a contiguous footprint of buildable hexes (13 and 19 hexes)
carrying **dozens of buildings** (43 and 55), plus a city DEFENCE scene (enemy
`city_3` army besieging the player capital), a city ATTACK scene (player army
besieging the enemy capital), and two open-field battles (`city_1`, `city_2`).
Totals: 98 buildings, 132 units.

**Why:** The old default (20 v 20 in open field) exercised almost none of the
city/building/siege code paths, so testing those needed manual setup every time.

**Impact:**
- New script `scripts/generate-default-scenario.js` (RNG seeded → reproducible).
  It contains a **faithful JS port** of the `shared/buildings.ts` street-navigation
  validators (`hasThroughStreet`, `findOrphanedPockets`, placement check); every
  building is greedily placed only if it passes that check, so cities keep legal
  through-streets and no sealed courtyards. Verified against the *compiled*
  `dist/shared/buildings.js` validators — both cities pass. **Keep the port in
  sync if `shared/buildings.ts` invariants change.**
- `client/worldData.ts` default fetch changed `/battle-20v20.json` →
  `/default-scenario.json`. `battle-20v20.json` is retained and both are listed
  in `client/saveLoad.ts` `BUNDLED_SAVES` (Load modal).
- Camera opens on the player capital (`battleCentreTile = 2123`) so the defence
  siege is front-and-centre.

**Decision:** Implemented the `building-damage` spec. Buildings are never removed
by combat. A successful Direct_Fire or Splash_Fire attack that reaches a building
strips exactly **one point from one component** (the seven equipment attributes).
Direct_Fire degrades the single targeted building's attacker-chosen component;
Splash_Fire degrades one **uniformly-random** eligible component of every enemy
building in the hex (resolved on the server) and still applies HP damage to enemy
units there. Anti_Air_Fire never targets buildings. Flat one point — no armour
mitigation, no min-damage formula. Auto weapon-mode against a building defaults to
Splash_Fire.

**Why:** Makes a city's footprint permanent while letting attackers wear down its
capabilities. Reverses the prior `COMBAT_RULES.md` §12 assumption that "buildings
are removed when destroyed."

**Impact:**
- `src/world/combat.ts`: new `BuildingComponent`, `BUILDING_COMPONENTS`,
  `BuildingDamageEvent`, `getEligibleBuildingComponents`,
  `applyBuildingComponentDamage`, `resolveBuildingDirectFire`,
  `resolveBuildingSplashInHex`, `resolveSplashHex`. `CombatResult` gained a
  `buildingDamage: BuildingDamageEvent[]` field; `resolveAttack` now also degrades
  enemy buildings sharing a splash target's hex (Resolved decision O1). Functions
  take an optional injectable `RandomFn` for deterministic server resolution.
- `server/combatApi.ts`: `CombatRequest` gained `targetBuildingId`, `component`,
  `weaponMode`; new `handleBuildingAttack`; responses carry `updatedBuildings`.
- `shared/combatTypes.ts`: `BuildingDamageReport`, `ExplainedCombat.buildingDamage`,
  `CombatResponse.updatedBuildings`.
- Client: `combatPanel.resolveAttack` returns `buildings`; `playerActions.ts` and
  `aiTurn.ts` sync component values and rebuild affected building models via
  `rerenderBuildingSprite`. `client/worldData.ts` validates persisted building
  component values (0–5) on load and rejects corrupt saves (Req 8.5).
- Authoritative rules updated in `COMBAT_RULES.md` §12 (new *Building Damage*
  subsection).
- **Client targeting UI:** right-clicking an in-range enemy building with a
  unit selected attacks it. Splash-only attackers fire immediately; when a
  choice exists (Splash vs Direct, or which component) a `BuildingAttackMenu`
  (`client/buildingAttackMenu.ts`) pops up — Splash first (default, Req 2.6),
  then a Direct Fire component list (the building's value ≥ 1 components). Wired
  via `localMap.onAttackBuilding` → `handlePlayerBuildingAttack`. Component
  identifiers live in `shared/buildingComponents.ts` (client can't import src/).
- **Attack animation:** building targets get the same missile → explosion FX as
  units, in both the 2D tactical map (`localMap.playBuildingAttackAnimation` +
  `getBuildingScreenPos`) and first-person view
  (`firstPersonView.playBuildingAttackAnimation` + `buildingWorldPos`).
  Splash_Fire on a building's hex also explodes/smokes any enemy units caught in
  it; the building itself never smokes (indestructible). The server's
  building-combat explanation reports those splash unit victims so the log and
  both animations share the same data.

## 2026-06-22 — ARCHITECTURE.md split into a scoped wiki

**Decision:** Split the monolithic `ARCHITECTURE.md` into a cross-linked wiki under
`docs/architecture/` (hub `README.md` + `modules.md`, `world-generation.md`,
`data-flow-and-api.md`, `configuration.md`, `debugging.md`, `known-issues.md`).
`ARCHITECTURE.md` is now a thin landing page that links into the wiki. Steering was
split to match: `architecture.md` is the hub (tech stack + module map, loads for
`src/**`, `server/**`, `shared/**`, configs), and `architecture-worldgen.md`,
`architecture-api.md`, `architecture-config.md`, `architecture-debugging.md` each
auto-load only their detail page via tighter `fileMatchPattern` scopes.

**Why:** The old steering inlined the entire doc whenever any backend/config file was
touched, so a config tweak loaded the full `window.gameDebug` instrumentation
reference. Scoping the pages cuts steering tokens to what the current edit needs.

**Impact:** Update wiki pages (not `ARCHITECTURE.md`) when architecture changes.
Cross-references updated in `README.md`, `.kiro/steering/{README,conventions,
ui-defaults,docs-as-we-go}.md`, and the `client/gameDebug.ts` header comment.

## 2026-06-22 — Combat functions take a CombatContext object

**Decision:** Replaced the positional `(allUnits, tiles, buildings)` threading in
the combat system with a single `CombatContext { units, tiles, buildings }`
object (defined/exported in `src/world/combat.ts`). Every combat function now
takes `ctx` in place of those positional args.

**Why:** Adding `buildings` to the EW calc (entry below) required editing ~15
call sites across 4 files purely to pass state through functions that didn't
even read it. With a context object, adding a future state source (e.g. cities,
weather) touches only the functions that read it plus the one place `ctx` is
built — not every signature in the chain.

**Impact:**
- `combat.ts`: `getEWProtection(target, ctx)`, `getDefencePower(target, ctx,
  attackerIsDrone?)`, `calculateDirectDamage`, `calculateSplashDamage`,
  `evaluateWeaponOptions`, `resolveAttack(attackerId, targetId, ctx)`,
  `calculateAntiAirReactionDamage`, `resolveAntiAirReactionFireForTile`,
  `resolveReactionFire`, `resolveSimultaneousAttacks` all take `ctx`. Bodies
  destructure `const { units: allUnits, tiles, buildings } = ctx;` so internal
  logic is unchanged — this is a pure signature refactor, no behaviour change.
- `combatExplainer.ts`: `explainAttack` / `explainSplash` / `buildBreakdown`
  take `ctx`.
- `combatApi.ts`: builds `ctx` once in `handleCombat` and passes it to every
  handler.
- Tests: a `ctx(units, tiles?, buildings?)` helper in `combat.test.ts` wraps the
  args; defaults tiles to the per-test grid and buildings to `[]`.
- No default-`[]` params anymore — `ctx.buildings` is always present.
- tsc clean (src/shared); 348 tests pass.

## 2026-06-22 — Buildings contribute to EW radius screens

**Decision:** Resolved the 2026-06-21 follow-up: EW-bearing **buildings** now
contribute to the radius anti-drone screen exactly like units. A friendly
building with `attributes.defence ≥ 1` projects `max(0, defence − hopDistance)`
to nearby friendly defenders, additive with unit screens.

**Why:** Buildings already carry `defence` and were drawn in the EW overlay, but
combat resolution ignored them — the screen you saw didn't match the protection
you got. This closes that gap.

**Impact:**
- `combat.ts`: `getEWProtection(target, allUnits, tiles, buildings = [])` now sums
  building sources too (buildings are alive while present — no `currentHealth`).
  `buildings` threaded as a trailing optional param through `getDefencePower`,
  `calculateDirectDamage`, `calculateSplashDamage`, `evaluateWeaponOptions`,
  `resolveAttack`, `calculateAntiAirReactionDamage`,
  `resolveAntiAirReactionFireForTile`, `resolveReactionFire`,
  `resolveSimultaneousAttacks`. Default `[]` keeps existing callers/tests intact.
- `combatExplainer.ts`: `explainAttack` / `explainSplash` take `buildings = []`.
- `combatApi.ts`: `CombatRequest.buildings?` (new `WireBuilding`), `rebuildBuildings`,
  passed into attack/preview/move resolution.
- `combatPanel.ts`: client sends `buildings: world.buildings` on preview/attack/move.
- COMBAT_RULES §12 updated (sources = units + buildings).
- tsc clean (src/shared); 348 tests pass; client diagnostics clean (5 pre-existing
  unrelated client tsc errors remain).
